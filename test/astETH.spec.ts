...
import { ERC20, LendingPool, StETHMocked } from '../types';
...
makeSuite('StETH aToken', (testEnv: TestEnv) => {
  beforeEach(async () => {
    ...
    const { users, stETH, deployer, pool, dai } = testEnv;
    ...
    await stETH.connect(deployer.signer).transfer(borrowerAAddress, await fxtPt(stETH, '1000'));
    await stETH.connect(deployer.signer).transfer(borrowerBAddress, await fxtPt(stETH, '1000'));
  });
  ...
  const printBalances = async (title: string = '') => {
    ...
    await astETH._rebasingIndex().then((v) => console.log('  astETH._rebasingIndex():', format(v)));
    await astETH
      .scaledTotalSupply()
  ...
  describe('multi user borrow repay', function () {
    it('rebases without borrows', async () => {
      ...
      const initialBalances = [
        balances.stETH.lenderA,
        balances.stETH.lenderB,
        balances.stETH.lenderC,
      ];

      // lender A deposits
      const lenderADepositAmount = 10_000;
      await deposit(pool, stETH, lenderA, lenderADepositAmount);
      await printBalances('Lender A Deposits stETH');
      await assertDeposits(
        stETH,
        astETH,
        initialBalances,
        [lenderA, lenderB, lenderC],
        [lenderADepositAmount, 0, 0]
      );
      ...
      // lender B deposits
      const lenderBDepositAmount = 5_000;
      await deposit(pool, stETH, lenderB, lenderBDepositAmount);
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [lenderADepositAmount, lenderBDepositAmount, 0]
      );
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [1.5 * lenderADepositAmount, 1.5 * lenderBDepositAmount, 0]
      );

      // lender C deposits
      const lenderCDepositAmount = 2_500;
      await deposit(pool, stETH, lenderC, lenderCDepositAmount);
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [1.5 * lenderADepositAmount, 1.5 * lenderBDepositAmount, lenderCDepositAmount]
      );
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [1.425 * lenderADepositAmount, 1.425 * lenderBDepositAmount, 0.95 * lenderCDepositAmount]
      );
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [1.425 * lenderADepositAmount, 1.425 * lenderBDepositAmount, 0]
      );
      ...
      await assertDeposits(
        ...
        initialBalances,
        [lenderA, lenderB, lenderC],
        [0, 1.425 * lenderBDepositAmount, 0]
      );
