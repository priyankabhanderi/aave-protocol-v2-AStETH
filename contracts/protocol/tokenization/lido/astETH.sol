...
interface IDebtStETH {
  function getStETHBorrowData() external view returns (uint256, int256);
}

// TODO: add detailed description for the token math.
contract astETH is VersionedInitializable, IncentivizedERC20, IAToken {
  ...
  // Keeps track of the shares deposited into the AAVE system
  ...
  constructor(
    ...
    STETH = ISTETH(UNDERLYING_ASSET_ADDRESS);
  ...
  function burn(
    ...
    _totalShares = _totalShares.sub(STETH.getSharesByPooledEth(amountScaled));
  ...
  function mint(
    ...
    _totalShares = _totalShares.add(STETH.getSharesByPooledEth(amountScaled));
  ...
  function _mintScaled(address user, uint256 mintAmountScaled, ExtData memory e) private {
    ...
    if (totalSupplyInternalBefore == 0) {
      _mint(
      user,
      STETH.getSharesByPooledEth(mintAmountScaled);
      );
    return;
    }
  ...
  function _burnScaled(address user, uint256 burnAmountScaled, ExtData memory e) private {
    ...
    if (otherBalanceScaledBefore == 0) {
      _burn(
        user,
        burnAmountScaled.mul(totalSupplyInternalBefore).div(scaledTotalSupplyBefore)
      );
      return;
    }
  ...
  /**
   * @dev w.r.t underlying ERC20 token
   **/
  function _scaledBalanceOf(uint256 _intBalanceOf, uint256 _intTotalSupply, uint256 _scaledTotalSupply) private pure returns (uint256) {
    if (_intBalanceOf == 0 || _intTotalSupply == 0) {
      return 0;
    }
    return _intBalanceOf.wayMul(_scaledTotalSupply).wayDiv(_intTotalSupply);
  }
   ...
   **/
  function _scaledTotalSupply(ExtData memory e) private pure returns (uint256) {
    return STETH.getPooledEthByShares(_totalShares - e.totalSharesBorrowed) + e.totalPrincipalBorrowed;
  }

  /**
   * @dev Helper method to convert stETH to shares
   **/
  function _stETHToShares(uint256 stETHAmount) private pure returns (uint256) {
    return STETH.getSharesByPooledEth(stETHAmount);
  }

  /**
   * @dev Helper method to convert shares to stETH
   **/
  function _sharesToStETH(uint256 sharesAmount) private pure returns (uint256) {
    return STETH.getPooledEthByShares(sharesAmount);
  }

  /**
   * @dev Queries external contracts and fetches data used for aTokenMath
   *      - total supply of stETH
   *      - principal borrowed and borrowed shares from the debt contracts
   **/
  function _fetchExtData() internal view returns (ExtData memory) {
    ExtData memory _extContractData;
    _extContractData.totalStETHSupply = IERC20(UNDERLYING_ASSET_ADDRESS).totalSupply();
    ...
    _extContractData.totalPrincipalBorrowed = variablePrincipal;
    _extContractData.totalSharesBorrowed = variablePrincipalScaled;
